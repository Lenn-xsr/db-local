...
const read = (path, packContent) => {
  let data = JSON.parse(readFileSync(path, { encoding: "utf-8" }));

  if (
    (typeof data == "string" && !packContent) ||
    (packContent && typeof data == "string")
  )
    data = unpack(data);
...
const save = (path, content, packContent) => {
  if (packContent) content = pack(content);
