...
function Methods(schema, path) {
  ...
  const find = (query) => {
    ...
    if (typeof query === "function") {
      ...
      data = Model.find((obj) => obj?.[queryParams?.[0]] === queryParams?.[1]);
    } else {
      data = Model.find((obj) => obj._id === query);
    }
  ...
  const remove = (query) => {
    ...
    if (typeof query === "function") {
      data = Model.filter((obj) => !query(obj));
    } else if (typeof query === "object") {
      const queryParams = Object.entries(query)?.[0];
      data = Model.filter(
      ...
      data = Model.filter((obj) => obj._id !== query);
    }
