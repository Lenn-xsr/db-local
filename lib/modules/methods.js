...
function Methods(schema, path, schemas) {
  ...
  const find = (query) => {
    ...
    const getAction = {
      function: () => Model.filter(query),
      object: () => {
        const queryParams = Object.entries(query)?.[0];
        const queryReferences = checkObjectReferences(query, schemas);
    ...
    return data ? dataWithProperties(data) : null;
  };
